# EXAMPLE TO DEMONSTRATE RESOURCE UNAVAILABLITY SCENARIOS
""" So far in our models, we've assumed that, outside of working
    on our modelled activities, our modelled resources are always
    available. But that won't always be the case in the real world
    Resources may not always be 'on shift' or may be called off to 
    other areas of the system (e.g different parts of the hospital)
    We can model this in SimPy by 'obstructing' a resource for a
    certain amount of time.

    Let's consider an example using our ED model. Let's imagine we
    want to model that an ED Doctor isn't always available for our
    part of the system. This may not be the same ED doctor in each
    case-it might just represent that we're down an ED doctor for
    an amount of time. We could set up an element of randomness
    around this (particularly if you're modelling a resource being
    'called away'), but in this example, we'll assume that we're 
    emulating shift patterns, so we'll use fixed periods of
    unavailability. Specifically, we'll set things up so that every
    8 hours, an ED doctor is unavailable for 4 hours. We will create
    new attribute values in class g:
    unavail_time_ed_doctor = 240 #4 hours
    unavail_freq_ed_doctor = 480 #8hours
"""
